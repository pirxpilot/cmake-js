...
Toolset.prototype.initialize = function (install) {
    if (!this._initialized) {
        if (environment.isWin) {
            this.initializeWin(install);
        }
        else {
...
Toolset.prototype.initializeWin = function (install) {
    // Visual Studio:
    // if it's already set because of options...
    if (this.generator) {
        if (install) {
    ...
    let topVS = this._getTopSupportedVisualStudioGenerator();
...
Toolset.prototype._getTopSupportedVisualStudioGenerator = function () {
    let CMake = require("./cMake");
    assert(environment.isWin);
    let list = CMake.getGenerators(this.options);
    let maxVer = 0;
    let result = null;
    for (let gen of list) {
